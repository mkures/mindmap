...
def api_schema():
    """Self-describing schema for external agents. Public (no auth)."""
    return jsonify({
        'version': '1.0',
        ...
        'auth': {
            ...
            'header': 'X-API-Key',
            'format': 'mk_<token>',
            'obtain': 'Generated by admin in /admin -> Users -> "Generate API key"'
        },
        'workflow': [
            'GET /api/schema - read this document',
            'POST /api/maps - create empty map, receive {id}',
            'GET /api/maps/<id>/outline - read current state as indented tree',
            'POST /api/maps/<id>/inject - apply batch of operations',
            'Loop outline->inject until done',
            'User opens /m/<id> in browser to view/edit'
        ],
        'endpoints': {
            'create_map': {
                'method': 'POST',
                ...
                'body': {
                    ...
                    'map': {
                        ...
                        'nodes': 'object<id, MindNode> - must contain rootId',
                        'settings': 'object (optional)'
                    }
                },
                'returns': {'id': 'map-xxxxx', 'title': 'string', 'updatedAt': 'number'},
                'errors': {
                    '409': 'A map with that title already exists (response includes existing_id)'
                }
            },
            'get_outline': {
                'method': 'GET',
                'path': '/api/maps/<map_id>/outline',
                'auth': True,
                'returns': {
                    'map_id': 'string',
                    'title': 'string',
                    'tree': 'string - indented outline with [id] prefixes',
                    'free_bubbles': 'array of {id, text, fx, fy, tags}',
                    'cards': 'array of {id, text, body, fx, fy, tags}',
                    'frames': 'array of {id, title, x, y, w, h, color}',
                    'tags': 'array of {id, name, color}'
                },
                'note': 'Use tree output to know existing node IDs before referencing them in inject.'
            },
            'inject': {
                'method': 'POST',
                'path': '/api/maps/<map_id>/inject',
                'auth': True,
                'body': {'operations': 'array of Operation (see operations below)'},
                'returns': {
                    'ok': True,
                    'operations_applied': 'number',
                    'operations_skipped': 'number',
                    'node_ids': 'object<requested_id, actual_id> - created nodes',
                    'errors': 'array of {index, op, error} (only if any failed)'
                },
                'semantics': 'Operations apply sequentially. IDs created in the batch can be referenced by later ops in the same batch.'
            }
        },
        'operations': {
            'add_child': {
                'description': 'Add a child node to an existing tree node.',
                'required': ['parent', 'text'],
                'optional': ['id', 'color', 'tags'],
                'example': {'op': 'add_child', 'parent': 'root', 'id': 'n1', 'text': 'Branch'}
            },
            'add_sibling': {
                'description': 'Add a sibling next to an existing tree node (cannot use root as sibling_of).',
                'required': ['sibling_of', 'text'],
                'optional': ['id', 'color', 'tags']
            },
            'add_free_bubble': {
                'description': 'Free-floating note at absolute position (not in tree).',
                'required': ['text'],
                'optional': ['id', 'fx', 'fy', 'color', 'tags'],
                'note': 'fx/fy in pixels, origin is canvas center. Default color #fef3c7.'
            },
            'add_card': {
                'description': 'Free-floating rich card with title + body.',
                'required': ['text'],
                'optional': ['id', 'fx', 'fy', 'body', 'cardWidth', 'cardExpanded', 'color', 'tags'],
                'note': 'cardWidth defaults to 280. body supports plain text.'
            },
            'add_link': {
                'description': 'Arrow/line between two nodes (can cross tree branches).',
                'required': ['from', 'to'],
                'optional': ['label', 'color', 'style'],
                'note': 'style: "dashed" | "solid". Duplicates are skipped.'
            },
            'add_frame': {
                'description': 'Decorative rectangle grouping a region of the canvas.',
                'required': [],
                'optional': ['id', 'title', 'color', 'x', 'y', 'w', 'h']
            },
            'add_tag': {
                'description': 'Define a tag in map settings (reusable across nodes).',
                'required': ['id'],
                'optional': ['label', 'color']
            },
            'update_node': {
                'description': 'Update text/body/tags/color of an existing node.',
                'required': ['id'],
                'optional': ['text', 'body', 'tags', 'color']
            },
            'delete_node': {
                'description': 'Delete a node and all descendants. Root cannot be deleted.',
                'required': ['id']
            }
        },
        'conventions': {
            'ids': 'Optional on create. Provide explicit id if you want to reference the node later in the same batch, otherwise one is generated (n1, n2, ...).',
            'colors': 'Hex format #rrggbb.',
            'coordinates': 'fx/fy/x/y in pixels. Origin (0,0) is canvas center. Positive X right, positive Y down.',
            'constraints': 'rootId cannot be deleted. sibling_of must not be the root. Duplicate links are silently skipped.'
        },
        'example_batch': {
            'request': {
                'operations': [
                    {'op': 'add_child', 'parent': 'root', 'id': 'n1', 'text': 'Objectifs'},
                    {'op': 'add_child', 'parent': 'n1', 'text': 'CA +20%'},
                    {'op': 'add_card', 'id': 'c1', 'fx': 400, 'fy': -200, 'text': 'Note', 'body': 'Details'},
                    {'op': 'add_link', 'from': 'n1', 'to': 'c1', 'label': 'cf.'}
                ]
            }
        }
    })
